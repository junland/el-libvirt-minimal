...
This repository uses a consolidated build process with a single entrypoint script (`build-rpms.sh`) that handles all aspects of RPM building:
...
# Run the build
docker run --rm -t rpm-builder
```

### Direct execution

The `build-rpms.sh` script can also be executed directly in an environment with:
